...
const EgressRecords = ({
  ...
  const getRecords = async () => {
    const response = await axios("http://localhost:9000/api/records/egress", {
      method: "get",
    });
  ...
  useEffect(() => {
    getRecords();
  }, [records, handleSubmitUpdate]);

  return (
    <div>
      <p className="h3 mb-4 text-center">Egress Records</p>
      {records ? (
